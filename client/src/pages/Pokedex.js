...
function Pokedex() {
  const [loadMore, setLoadMore] = useState('http://localhost:8006/api/v1/pokemons?limit=20&offset=0');
  ...
  const getPokemonObject =  () => {
    ...
    axios.get(loadMore).then(response =>{
      setOffset(offset + 20);
      setLoadMore(`http://localhost:8006/api/v1/pokemons?limit=20&offset=${offset}`); 
      ...
      <div className='allPokemons'>
        {pokemonObject.map(pokemonData =>
            <Link to={`/pokemon/${pokemonData.name}`}>
                <PokedexCard key={pokemonData.id} url={pokemonData.img} name={pokemonData.name} type={pokemonData.type} className='pokemonCard' />
            </Link>
        )}
      </div>
